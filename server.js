...
Server.prototype.removeObject = function(obj) {
  ...
  this.emitTo(room.name, "r", this._getRemovePacket(obj));
...
Server.prototype.packetUpdate = function() {
  ...
  this.rooms.forEach((room, key) => {
    const objs = [];
    for (let i = 0; i < room.objects.length; i++) {
      const pack = this._getUpdatePacket(room.objects[i]);
...
Server.prototype.start = function(options={}) {
  ...
  this.ws.on('connection', (ws, req) => {
    ...
    ws.join = (roomName) => {
      ...
      if (oldRoom) {
        //remove from oldRoom client list
        if (oldRoom.clients.indexOf(ws) !== -1) oldRoom.clients.splice(oldRoom.clients.indexOf(ws), 1);

        //have to send remove packets for all objects in prev room
        for (let i = 0; i < oldRoom.objects.length; i++) {
          this.emitTo(ws, 'r', this._getRemovePacket(oldRoom.objects[i]));
        }
      }
